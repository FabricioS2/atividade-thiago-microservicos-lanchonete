...
# # from .external_services import obter_disponibilidade, processar_pagamento
# # import aio_pika
# # import httpx
...
# # # Endpoint simplificado – não tratamos autenticação, apenas lemos X-User-Id
...
# #     # 1. Validar itens com o cardápio
# #     itens_validados = []
...
# #         preco = disp["preco"]
# #         # Buscar nome? Poderíamos ter outro endpoint, mas vamos confiar que preço veio.
# #         # Na real faríamos outra chamada GET /itens/{id}. Vamos simplificar: assumir nome do cardápio.
# #         # Para o exercício, pegaremos o nome depois. Vamos fazer a chamada de detalhe:
# #         # async with httpx... pegar nome; aqui farei uma chamada extra, mas para simplificar, colocarei "nome temporário"?
# #         # Melhor: faremos outra chamada GET /itens/{id} para obter nome.
# #         # Vou adicionar essa função em external_services.
# #         # Vou incluir um fallback: se a função não existir, usamos "Nome Indisponível".
# #         # Para não alongar demais, vou criar a função obter_item_nome.
# #         # Na prática, adicionaremos ao external_services:
# #         # async def obter_item(item_id): retorna json.
# #         # Vou adicionar rapidamente.
# #         pass
    
# #     # Vou completar com código mais realista:
...
# #             resp = await client.get(f"{CARDAPIO_URL}/itens/{item_req.item_id}")
...
# #         db.commit()
# #         # Publica mensagem no RabbitMQ para notificar a cozinha
# #         try:
...
# #                     ),
# #                     routing_key="cozinha.notificar"
# #                 )
# #         except Exception as e:
# #             # Log, mas não falha o pedido
...
#     novo_pedido = Pedido(cliente_id=x_user_id, status="CRIADO")
#     db.add(novo_pedido)
#     db.flush()  # para obter o ID antes de inserir os itens
...
#         # 2. Obter nome e preço atuais do cardápio
...
#             channel = await connection.channel()
#             exchange = await channel.declare_exchange("pedidos", aio_pika.ExchangeType.TOPIC, durable=True)
#             # Garante que a fila existe (pode ser criada aqui para evitar perda)
